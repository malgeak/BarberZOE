	...
	*/
	...
	switch ($tipo) {
		case 1: //Insertar en productos
			...
			$resultado = mysqli_query($conexion, $query) or die("Error al insertar registros a productos");
		...
		case 2: //Insertar en materiales
			$query = "INSERT INTO $tabla(fkinventario, pkserial, nombre, marca, precio, stock) VALUES ($tipo, $place, '$nombre', '$marca', $precio, $stock)"; //Inserta datos
			$resultado = mysqli_query($conexion, $query) or die("Error al insertar registros a materiales");
			echo "Se insertó en la tabla ".$tabla.", el elemento con el ID ".$place.".";
		...
		case 3: //Insertar en ventas
			...
			if(empty($elegidos)){ //En caso de no elegir alguno para venta
				...
				for($i=0; $i < $counts; $i++){ //El $elegidos[$i] seleccionado es el pkserial
					...
					$resultado = mysqli_query($conexion, $query) or die("Error al obtener datos de productos para venta");					
					while ($fila=mysqli_fetch_array($resultado)) {
				...
				$resultado = mysqli_query($conexion, $query) or die("Error al insertar datos en ventas");
				echo "Se insertó en la tabla ".$tabla.", el elemento con el ID ".$place.", ";
				echo $descripcion.".";
		...
		case 4: //Insertar en gastos
			...
			$resultado = mysqli_query($conexion, $query) or die("Error al insertar registro a gastos");
			echo "Se insertó en la tabla ".$tabla.", el elemento con el ID ".$place.".";
		...
		case 5: //Insertar en eventos
			...
			$resultado = mysqli_query($conexion, $query) or die("Error al insertar registro a eventos");
			echo "Se insertó en la tabla ".$tabla.", el elemento con el ID ".$place.".";
